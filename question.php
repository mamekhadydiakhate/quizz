<?php
	...
	$questions = recupJson('question');

	$question = $type = $point = null;
	$reponses = $reponsesVraies = [];
	if (isset($_POST['valider']))
	{
		$question=$_POST['question'];
		$point=$_POST['point'];
		$type = $_POST['typedereponse'];
		foreach($_POST as $cle => $valeur) // $_POST = [$key => $val, 1 => 'odc', 2 => 'simplon', 4]
		{
			if(preg_match('/^(reponse)/',$cle)){
				$reponses[] = $valeur;
				$id = explode('_',$cle)[1];
				if(isset($_POST["checkbox$id"]) || ($_POST["radio"]))
				{
					$reponsesVraies[] = $valeur;
				}
			}
		}
		
		if(!empty($question && $point && $type && $reponses && $reponsesVraies))
		{
			$questions[] = 
			[
			'question' => $question, 
			...
			$quizzquestion=json_encode($questions,JSON_PRETTY_PRINT);
			file_put_contents('question.json',$quizzquestion);
	
		}
		if (isset($_POST["checkbox"])) 
		{
			$reponsesVraies<$reponses;
		}
		else {
			echo'on doit pas choisir toute les reponse';
		}
	}
?>
        ...
        <div>
            ...
            <div class="typereponse">
                ...
                <select name="typedereponse" id="type" placeholder="Donner le type de reponse">
                    ...
                    <option value="choix_multiple" name="option1">choix multiple</option>
                    <option value="choix_simple" name="option2">choix simple</option>
                    <option value="reponse_texte" name="option3">reponse texte</option>
                </select>
		...
		if (x==""){
			...
			switch(x) {
				...
				default:
				    ...
				    valeur.setAttribute("name","checkbox"+i);
			}
